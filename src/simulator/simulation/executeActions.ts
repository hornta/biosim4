...
export const executeActions = (indiv: Indiv, actionLevels: number[]) => {
  ...
  if (isEnabled(SensorAction.SetOscilliatorPeriod)) {
    let periodf = actionLevels[SensorAction.SetOscilliatorPeriod];
    let newPeriodf01 = (Math.tanh(periodf) + 1.0) / 2.0; // convert to 0.0..1.0
    let newPeriod = 1 + Math.floor(1.5 + Math.exp(7.0 * newPeriodf01));
  ...
  if (
    ...
    if (level > killThreshold && prob2bool(indiv.simulation.random, level)) {
      let otherLoc = indiv.location.addDirection(indiv.lastMoveDirection);
      if (
  ...
  let level;
  let offset;
  let lastMoveOffset = indiv.lastMoveDirection.asNormalizedCoord();
  ...
  let probX = prob2bool(indiv.simulation.random, Math.abs(moveX)); // convert abs(level) to 0 or 1
  let probY = prob2bool(indiv.simulation.random, Math.abs(moveY)); // convert abs(level) to 0 or 1
