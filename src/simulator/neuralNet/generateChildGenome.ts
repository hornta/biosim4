import { Genome, makeRandomGenome } from "./gene.js";
...
const randomBitFlip = (simulation: Simulation, genome: Genome) => {
  let elementIndex = getRandomInt(simulation.random, 0, genome.length - 1);
  ...
  if (chance < 0.2) {
    ...
    let bitIndex8 = 1 << getRandomInt(simulation.random, 0, 7);
    // sourceNum
...
const cropLength = (simulation: Simulation, genome: Genome, length: number) => {
  if (genome.length > length && length > 0) {
    if (simulation.random.uint32() / RANDOM_UINT_MAX < 0.5) {
      // trim front
      let numberElementsToTrim = genome.length - length;
...
const randomInsertDeletion = (simulation: Simulation, genome: Genome) => {
  let probability = simulation.options.geneInsertionDeletionRate;
...
export const generateChildGenome = (
  ...
  assert(genome.length !== 0);
  assert(genome.length <= simulation.options.genomeMaxLength);
  return genome;
};
